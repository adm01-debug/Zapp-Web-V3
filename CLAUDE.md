...
| ~~BUG-9~~ | `zapp.rpc_dlq_retry_now` no DB | CORRIGIDO 2026-07-17: overload (uuid) sem role check + usava coluna `next_retry_at` (inexistente); overload (uuid,uuid) tinha parâmetros em ordem invertida (`p_item_id, p_id`). Migration `20260717_fix_dlq_security_and_audit_gaps.sql` DROP+CREATE com `zapp.has_role` e `next_attempt_at` correto | Resolvido |
| ~~BUG-10~~ | `zapp.rpc_dlq_abandon` no DB | CORRIGIDO 2026-07-17: 2 overloads inseguros (uuid) e (uuid,uuid) sem role check nem audit log. Drops aplicados; canonical (uuid,text) corrigido com `zapp.has_role` + INSERT em `zapp.audit_logs` | Resolvido |
| ~~BUG-11~~ | `zapp.rpc_dlq_bulk_abandon` no DB | CORRIGIDO 2026-07-17: overload (uuid[]) sem role check, tipo de retorno errado (boolean vs integer). Drop aplicado; canonical (uuid[],text) corrigido com `zapp.has_role` + audit log | Resolvido |
