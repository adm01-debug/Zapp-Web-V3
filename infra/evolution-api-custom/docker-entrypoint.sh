...
if [ -f /run/secrets/supabase_service_key_v1 ]; then
  ...
  if [ -n "$_aud_key" ]; then
    # OCI_DIGEST deve ser injetado via env no docker-compose/stack (não disponível em /proc em Alpine)
    _aud_digest="${OCI_DIGEST:-}"
    ...
    _aud_payload="{\"instance_name\":\"wpp2\",\"booted_at\":\"$_aud_ts\",\"image_digest\":\"$_aud_digest\",\"evolution_version\":\"$_aud_version\",\"logpatch_status\":\"ok\",\"patch_mode\":\"build-time\",\"logpatch_detail\":{\"mode\":\"build-time\",\"note\":\"patches T1-T6 aplicados em build (Dockerfile VERIFY fail-closed)\"}}"
