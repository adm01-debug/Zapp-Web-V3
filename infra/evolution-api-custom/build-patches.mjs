 ...
 *
 * Aplica os patches T1-T5 no bundle ORIGINAL /evolution/dist/main.js da
 ...
 * T2  remove console.log("stanza",JSON.stringify(e)),
 ...
 *
 * Uso:
 *   node build-patches.mjs [main.js] [main.patched.js] [t4_prologue.cjs]
 *   (defaults: ./main.js ./main.patched.js ./t4_prologue.cjs)
 */
import fs from "node:fs";
import path from "node:path";
import crypto from "node:crypto";

const USAGE = `Uso: node build-patches.mjs [main.js] [main.patched.js] [t4_prologue.cjs]
(defaults: ./main.js ./main.patched.js ./t4_prologue.cjs)`;
...
const [srcArg, outArg, prologueArg] = process.argv.slice(2);
...
// --- T2 (c2 deve ser 1) ---
{
  ...
  if (c !== 1) fail(`T2: target encontrado ${c}x (esperado 1x) — bundle da base mudou?`);
  out = out.split(T2).join("");
  applied.push("T2");
}

// --- T3 (init do Sentry deve ser único) ---
{
...
// --- T5a (console.log("CACHE:") deve ser único) ---
{
  const c = countOf(out, T5_COND);
  if (c !== 1) fail(`T5a: console.log("CACHE:...") encontrado ${c}x (esperado 1x) — bundle da base mudou?`);
