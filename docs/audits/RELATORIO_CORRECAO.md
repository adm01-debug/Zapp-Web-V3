...
| **E02-N21** | `QUEBRADO` | **O INV-6 que eu mesmo criei reprovou o PR #713 por motivo ambiental — corrigido no mesmo dia.** O #713 não toca banco nem migração (é uma correção de 21 linhas no `ProtectedRoute`), e mesmo assim o `DB Invariants` ficou vermelho no passo 8. Os INV-1 a INV-5 passaram. Diagnóstico limitado: `GET /actions/jobs/{id}/logs` responde `403 Must have admin rights` com o token do MCP, então a causa exata não foi lida — mas o caminho "sem secret → exit 0" foi reproduzido localmente com `bash -e -o pipefail` e sai 0, o que descarta erro de sintaxe e indica que o secret `SUPABASE_DB_URL` **existe** e a falha veio de `psql` (ausente no runner, sem conectividade, ou instalação falhando). **Correção:** o passo passou a separar falha de **infra** de falha de **invariante** — sem secret, sem `psql` ou sem conectividade emite `::warning` e passa; com conexão de pé, o check é bloqueante de verdade. Sonda de conectividade (`psql -Atc \x27SELECT 1\x27`) antes do check real. Ambos os caminhos testados localmente com `bash -e -o pipefail`, exit 0. **Isto não é desligar gate que pegou defeito real** — a publicação foi medida por MCP no mesmo dia (68 tabelas, 0 faltando, 0 extras). É impedir que um gate reprove por motivo ambiental, que é como se treina um time a ignorar gate. |
