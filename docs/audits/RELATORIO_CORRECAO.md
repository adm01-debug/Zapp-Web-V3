...
| **E02-N10** | `QUEBRADO` | **Passo 0 da Etapa 3 reprovou por causa disto.** O `testMatch` do project `public` em `playwright.a11y.config.ts` (F10-05) ficou `**/auth-*.spec.ts` — largo demais. Arrastou `auth-flow.spec.ts`, `auth-extended.spec.ts` e `auth-session-lifecycle.spec.ts` (specs que exigem backend real) para dentro do job `Accessibility regression (axe)` do `ci.yml`, que roda **sem credenciais**. Efeito medido: `CI/CD Pipeline` verde em `7f5519348` -> **failure** em `42a6ef0bb` e `ff04b78cb`, com o job de axe como unico reprovado. Efeito colateral inverso: no mesmo movimento `chat-accessibility.spec.ts` **deixou de rodar** nesse job, ou seja o gate que F10-05 queria alargar ficou ao mesmo tempo quebrado e mais estreito. Corrigido nesta sessao com `PUBLIC_A11Y = /auth-.*(accessibility\|keyboard-navigation)\.spec\.ts$/` — continua por padrao, nao nominal, como F10-05 pedia. **O gate nao foi desligado.** |
