...
- **Teto de 10 subagentes simultâneos.** Com esse volume, a **declaração de propriedade de arquivo deixa de ser recomendação e vira pré-condição**: antes de despachar a rodada, monte a tabela `agente | arquivos que vai tocar` e verifique interseção **par a par**. Se duas listas se cruzam, os dois agentes entram em rodadas diferentes. Rodada com interseção não é despachada.
- **Nomeie a rodada.** Cada leva paralela recebe um identificador (`R1`, `R2`, …) registrado em `RELATORIO_CORRECAO.md` com a lista de agentes, arquivos e achados. Se aparecer regressão, é o que permite descobrir qual agente a introduziu — com 10 frentes, sem esse registro o `git bisect` é o único recurso e ele custa caro.
- **Reconcilie em subgrupos antes do todo.** Com 10 diffs, aplique e teste em blocos de ~3 antes de rodar a suíte completa; só então rode `npx tsc --noEmit` e a suíte inteira com tudo junto. Verde individualmente não garante verde em conjunto — e falha no bloco isola o culpado, falha só no final não.
- **Paralelismo respeita o caminho crítico.** `3→16`, `6→7`, `13→14,19`, `F9-10→F9-09` são sequenciais e **não paralelizam, em nenhuma hipótese** — teto alto não afrouxa dependência. Onde os 10 rendem: **18** (28 achados de admin, independentes, arquivos distintos — o melhor caso), **15**, **20** e **9**.
- **Etapas de risco alto rodam com poucos agentes, mesmo com teto de 10.** A **Etapa 6** (view `zapp.contacts`, causa-raiz de 5 achados espalhados por 3 etapas) e a **Etapa 15** (inbox, onde ficam os homônimos de arquivo que já geraram erro de referência) pedem 1-2 agentes e revisão apertada. Teto é limite superior, não meta.
