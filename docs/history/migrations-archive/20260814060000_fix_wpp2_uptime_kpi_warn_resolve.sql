-- Migration: fn_wpp2_uptime_kpi — resolver alertas critical quando uptime sai da zona crítica
-- Detectado em auditoria 2026-08-14 07h BRT (Passo 1 dos próximos passos)
--
-- Lógica anterior: só resolvia alertas quando status='OK' (uptime>=99%).
-- Em WARN (95-99%): alerta CRITICAL permanecia aberto indefinidamente mesmo
-- o downtime já superado (histórico: uptime 98.08% ficou aberto ~3h30).
--
-- Fix: diferenciar resolução por severity:
--   WARN (>95%): resolve alertas severity='critical' (saiu da zona crítica)
--   OK  (>=99%): resolve tudo (critical + high + webhook_health_alerts)
--
-- NOTA: Parâmetros da nova assinatura: p_instance TEXT (1º), p_window INTERVAL (2º).
-- Ordem anterior era (p_window, p_instance). Crons e calls sem argumento não são afetados.
-- Overload anterior (oid 1482469) dropado: DROP FUNCTION evo.fn_wpp2_uptime_kpi(interval,text,boolean)
...
BEGIN
  ...
  IF p_alert THEN
    -- WARN: uptime saiu da zona crítica (>95%) — resolve alertas severity=critical
    -- OK:   uptime acima de 99% — resolve tudo (critical + high + webhook_health_alerts)
...
DROP FUNCTION IF EXISTS evo.fn_wpp2_uptime_kpi(interval, text, boolean);
