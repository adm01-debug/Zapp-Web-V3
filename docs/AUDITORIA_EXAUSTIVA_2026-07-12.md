...
17b. 🔴 **[DB][G]** **Novo achado (revisão automatizada, confirmado por leitura de migrations):** `useEmail.ts:loadAccounts()` lê da tabela `public.email_accounts` (criada em `20260531143555`), mas todo o resto do pipeline Gmail — `gmail-oauth` (`exchangeCode`), `gmail-sync`, `gmail-send`, `gmail-token-refresh`, `gmail-webhook`, e as tabelas de threads/mensagens (`email_app.email_threads.gmail_account_id`) — opera sobre `gmail_accounts`. A view `v_email_accounts_unified` (que lê de `gmail_accounts`) existe mas **não é consultada em nenhum lugar do frontend**. Há ainda duas definições concorrentes de `gmail_accounts` nas migrations (`20260403024714` com `profile_id/email_address/token_expires_at` vs. `20260502000001_gmail_tables_ensure` com `user_id/email/watch_expiry`, esta como `CREATE TABLE IF NOT EXISTS` — não aplicaria se a primeira já existisse) e uma menção a `email_app.gmail_accounts` (schema separado) em `20260702_security_hardening_m1_m7`. **Efeito prático suspeito:** conectar uma conta Gmail via OAuth pode nunca aparecer na lista de contas do inbox, pois `loadAccounts()` consulta uma tabela desconectada do restante do pipeline. **Não corrigido nesta sessão** — requer inspeção do schema *live* (`list_tables`/`describe_table` via MCP Supabase, indisponível neste ambiente por falta de autorização OAuth) para confirmar qual tabela/coluna é a fonte de verdade atual antes de qualquer mudança de código; um fix às cegas arrisca trocar por uma tabela/coluna que não existe de fato no banco em produção.
