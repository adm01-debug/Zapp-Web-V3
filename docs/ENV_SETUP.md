...
| `VITE_SUPABASE_ANON_KEY`                            | ✅          | Chave **anon** (role `anon`) do projeto — copie do painel Supabase (Settings → API). **NUNCA** use a chave `service_role` aqui: variáveis `VITE_*` vão para o bundle público. Obtenha o valor via secret manager; não versione. |
