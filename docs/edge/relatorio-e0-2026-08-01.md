...
- **Exploit confirmado:** função VIVA em `https://uqysyzndkfiwfztbqvsl.supabase.co/functions/v1/migrate-helper`:
  - `GET ?action=ping` com `x-access-key: 7bdebc20…(chave comprometida — rotação pendente; ver relatorio-e4)` → `{"ok":true,"project_ref":"https://uqysyzndkfiwfztbqvsl.supabase.co",...}` (200)
