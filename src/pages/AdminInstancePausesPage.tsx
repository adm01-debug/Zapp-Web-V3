...
import {
  ...
  type IncidentPause,
} from '@/features/admin/components/instance-pauses/IncidentDetailDialog';
  ...
  const pauseMut = useMutation({
    mutationFn: () =>
      invoke('pause', {
        ...
        minutes,
        reason: reason.trim() || 'manual_pause',
      }),
    onSuccess: () => {
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Suspende temporariamente o webhook e o envio Evolution para uma instância quando há
            picos de <code>invalid_signature</code> ou <code>auth 401/403</code>.
          </p>
        </div>
        <Button
          variant="outline"
          size="sm"
          onClick={() => qc.invalidateQueries({ queryKey: queryKeys.adminOps.instancePauses() })}
        >
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Pausas ativas</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Auto-pausas / 24h</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">Histórico (50 últimas)</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      {autoCount24h > 0 && (
        <Alert variant="default" className="border-warning/40 bg-warning/5">
          ...
          <AlertDescription>
            {autoCount24h} pausa(s) automática(s) nas últimas 24h por excesso de falhas de
            autenticação. Verifique a Webhook Secret e as credenciais da Evolution API.
          </AlertDescription>
        </Alert>
      )}
          ...
          <div className="grid grid-cols-1 items-end gap-3 md:grid-cols-4">
            <div>
              ...
              <Input
                id="instance"
                value={instance}
                onChange={(e) => setInstance(e.target.value as WhatsAppInstance)}
                placeholder="wpp2"
              />
            </div>
            <div>
              ...
              <Input
                ...
                value={minutes}
                onChange={(e) =>
                  setMinutes(Math.max(1, Math.min(1440, Number(e.target.value) || 1)))
                }
              />
            </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Pausas ativas agora</CardTitle>
        </CardHeader>
        <CardContent>
          {activeQuery.isLoading ? (
                  ...
                  {active.map((p) => (
                    ...
                    >
                      ...
                      <td className="py-2 pr-4 text-xs">
                        {formatDistanceToNow(new Date(p.paused_until), {
                          addSuffix: true,
                          locale: ptBR,
                        })}
                      </td>
