...
export const QueryExplorerBlock = forwardRef<QueryExplorerHandle>(
  function QueryExplorerBlock(_props, ref) {
    ...
    const initialParams = useMemo(
      () =>
        JSON.stringify(
          RPCS.find((r) => r.name === 'rpc_dashboard_home')?.exampleParams ?? {},
          null,
          2
        ),
      []
    ...
    const addFilter = () =>
      setFilters((f) => [...f, { _id: Date.now(), column: '', operator: 'eq', value: '' }]);
          ...
          <Tabs
            ...
            onValueChange={(v) =>
              setMode(
                v as
                  | 'select'
                  | 'rpc' /* ignore-audit: Select/Tabs value string narrowed to union; developer controls option values */
              )
            }
          >
            <TabsList>
              ...
              <div>
                ...
                {filters.length === 0 && (
                  <p className="text-xs text-muted-foreground">
                    Sem filtros — retorna últimos N registros.
                  </p>
                )}
                    ...
                    <div key={f._id} className="grid grid-cols-[1fr_120px_1fr_auto] gap-2">
                      ...
                      <Select
                        ...
                        onValueChange={(v) =>
                          updateFilter(i, {
                            operator:
                              v as Operator /* ignore-audit: Select/Tabs value string narrowed to union; developer controls option values */,
                          })
                        }
                      >
                        <SelectTrigger>
            ...
            <TabsContent value="rpc" className="space-y-3 pt-3">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full rounded-md border bg-background p-2 text-xs focus:outline-none focus:ring-2 focus:ring-ring"
                />
                {paramsError && (
                        ...
                        return (
                          <tr key={rowKey} className="border-t hover:bg-muted/50">
                            {columns.map((c) => {
                              ...
                              const display =
                                v == null ? '—' : isObj ? JSON.stringify(v) : String(v);
                              const truncated =
                                display.length > 80 ? display.slice(0, 80) + '…' : display;
                              return (
                                <td key={c} className="max-w-xs p-2 align-top" title={display}>
                                  <span className={isObj ? 'text-muted-foreground' : ''}>
                                    {truncated}
                                  </span>
                                </td>
                              );
