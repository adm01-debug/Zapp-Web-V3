...
export default function AdminQueuesPage() {
  ...
  const load = async () => {
    ...
    const [q, m, s, p, d, c, cq] = await Promise.all([
      safeClient.from<Queue>('queues', (q) =>
        q.select('*').order('priority', { ascending: false })
      ),
      supabase
        .from('queue_members')
        .select('id,queue_id,profile_id,profile:profiles(id,name,avatar_url)'),
      ...
      safeClient.from<ServiceChannel>('service_channels', (q) =>
        q.select('id,name,channel_type,default_queue_id').neq('status', 'archived').order('name')
      ),
      safeClient.from<ChannelQueue>('channel_queues', (q) => q.select('*')),
    ]);
  ...
  const save = async () => {
    ...
    const { error } = await safeClient.from('queues', (q) =>
      editing.id ? q.update(payload).eq('id', editing.id) : q.insert(payload)
    );
    if (error) {
  ...
  const linkChannel = async () => {
    ...
    const { error } = await supabase.rpc(
      'rpc_link_channel_queue' as never,
      {
        p_channel_id: newChannelId,
        p_queue_id: memberDialog.id,
        p_priority: 0,
        p_is_active: true,
      } as never
    );
  ...
  const unlinkChannel = async (channelId: string) => {
    ...
    const { error } = await supabase.rpc(
      'rpc_unlink_channel_queue' as never,
      {
            ...
            <Button
              onClick={() =>
                setEditing({
                  ...
                  status: 'active',
                  distribution_algorithm: 'least_busy',
                })
              }
            >
