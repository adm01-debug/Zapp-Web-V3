...
export default function AdminTelemetriaPage() {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      doc.text(
        `Exportado em ${now.toLocaleString('pt-BR')} · Período: ${periodLabels[timeFilter]} · ${rows.length} registros`,
        14,
        22
      );
      const headers = [
        ...
        <TabsContent value="server" className="mt-4 space-y-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Select
              ...
              onValueChange={(v) =>
                setSeverityFilter(
                  v as SeverityFilter /* ignore-audit: Select/Tabs value string narrowed to union; developer controls option values */
                )
              }
            >
