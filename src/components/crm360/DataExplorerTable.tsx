...
import { useExternalTableBrowser } from '@/hooks/useExternalApiManagement';
...
function RFMBadge({ segment }: { segment: string | null }) {
  ...
  return (
    <Badge
      variant="outline"
      className={RFM_SEGMENT_COLORS[segment] || 'bg-muted text-muted-foreground'}
    >
      {segment}
    </Badge>
  );
          ...
          <Table>
            ...
            <TableBody>
              {browser.isLoading ? (
                ...
                <TableRow>
                  <TableCell
                    colSpan={tabConfig.columns.length + 1}
