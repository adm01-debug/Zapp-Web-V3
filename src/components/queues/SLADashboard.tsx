...
const PERIOD_KEYS: PeriodFilter[] = ['today', 'week', 'month', 'all'];
...
export const SLADashboard = () => {
  ...
  const sparkFR =
    historyData?.dailyData.map((d) =>
      d.totalConversations > 0 ? 100 - (d.firstResponseBreaches / d.totalConversations) * 100 : 100
    ) || [];
  const sparkRes =
    historyData?.dailyData.map((d) =>
      d.totalConversations > 0 ? 100 - (d.resolutionBreaches / d.totalConversations) * 100 : 100
    ) || [];
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent) => {
      ...
      if (e.key >= '1' && e.key <= '4') {
        e.preventDefault();
        setPeriod(PERIOD_KEYS[parseInt(e.key) - 1]);
      }
      if (e.key === 'h' || e.key === 'H') {
        e.preventDefault();
    ...
    return (
      ...
      >
        <div className="relative mb-6">
          ...
          <div className="absolute -bottom-1 -right-1 flex h-8 w-8 items-center justify-center rounded-full bg-primary/10">
        ...
        <p className="max-w-md text-center text-sm leading-relaxed text-muted-foreground">
          Os dados aparecerão aqui quando conversas forem monitoradas pelo sistema de SLA. Configure
          prazos na aba de configuração abaixo para começar.
        </p>
        <Button
          variant="outline"
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Métricas de tempo de resposta e resolução</p>
        </div>
        <div className="flex items-center gap-3">
          <ExportButton getData={getExportData} />
          <Button variant="outline" onClick={() => navigate('/sla/history')} className="gap-2">
            ...
            <CardContent className="space-y-6">
              <div>
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <span
                    className={cn(
                      'text-sm font-bold',
                      getRateColor(data.overall.firstResponse.rate)
                    )}
                  >
              ...
              <div>
                <div className="mb-2 flex items-center justify-between">
                  ...
                  <span
                    className={cn('text-sm font-bold', getRateColor(data.overall.resolution.rate))}
                  >
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                <motion.div
                  ...
                  className="rounded-xl border border-destructive/20 bg-destructive/10 p-4 transition-colors hover:bg-destructive/15"
                >
                ...
                <motion.div
                  ...
                  className="rounded-xl border border-success/20 bg-success/10 p-4 transition-colors hover:bg-success/15"
                >
