...
export function InstanceSettingsDialog({ open, onOpenChange, instanceName, connectionName, connectionId }: InstanceSettingsDialogProps) {
  ...
  const loadProfile = async () => {
    try { const data = await fetchProfile(instanceName); if (data && mountedRef.current) setProfile({ name: data.name ?? '', status: data.status ?? '', pictureUrl: data.profilePictureUrl ?? '' }); }
