...
export function EmailAccountSelector({
  ...
  const activeStatus = activeAccountId
    ? (tokenStatus[activeAccountId] ?? 'disconnected')
    : 'disconnected';
  ...
  return (
    <div
      className={cn(
        'flex items-center gap-2 rounded-xl border border-border/10 bg-background/40 p-1 shadow-sm',
        className
      )}
    >
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button
            type="button"
            className="group flex min-w-0 flex-1 items-center gap-2.5 rounded-lg px-2.5 py-2 transition-all duration-200 hover:bg-muted/80"
          >
            <div className="relative shrink-0">
              <Avatar className="h-8 w-8 border border-primary/10 transition-colors group-hover:border-primary/30">
                <AvatarImage
                  src={activeAccount?.picture_url ?? ''}
            ...
            <div className="min-w-0 flex-1 text-left">
              <p className="mb-1 truncate text-[13px] font-bold leading-none tracking-tight">
            ...
            <div className="flex shrink-0 items-center gap-1.5">
              {totalUnread > 0 && (
                <Badge className="h-4.5 min-w-[18px] border-none bg-primary px-1 text-[9px] font-black text-primary-foreground">
                  {totalUnread > 99 ? '99+' : totalUnread}
                </Badge>
              )}
              <ChevronDown className="h-3.5 w-3.5 text-muted-foreground/50 transition-colors group-hover:text-foreground" />
            </div>
          </button>
        </DropdownMenuTrigger>
        ...
        <DropdownMenuContent
          align="start"
          className="w-64 border-border/30 bg-card/95 shadow-2xl backdrop-blur-xl"
        >
          {/* Status do token */}
          <div className={cn('flex items-center gap-2 px-3 py-2 text-xs', statusConfig.color)}>
            <StatusIcon
              className={cn('h-3.5 w-3.5 shrink-0', activeStatus === 'loading' && 'animate-spin')}
            />
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-xs font-medium">{acc.display_name ?? acc.email}</p>
