    ...
    if (searchQuery) {
      ...
      result = result.filter(
        (a) =>
          ...
          a.achievement_description?.toLowerCase().includes(q) ||
          a.achievement_type.toLowerCase().includes(q)
      );
    }
    const now = new Date();
    if (filterBy === 'today') {
      ...
      result = result.filter(
        (a) => new Date(a.earned_at) >= new Date(now.getTime() - 7 * 86400000)
      );
    else if (filterBy === 'month')
      result = result.filter(
        (a) => new Date(a.earned_at) >= new Date(now.getTime() - 30 * 86400000)
      );
      ...
      <CardHeader className="pb-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-warning to-warning shadow-lg shadow-warning/30">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="gap-2">
                ...
                {sortBy === 'recent' ? 'Mais Recentes' : sortBy === 'xp' ? 'Maior XP' : 'Por Tipo'}
                <ChevronDown className="h-3 w-3" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent>
