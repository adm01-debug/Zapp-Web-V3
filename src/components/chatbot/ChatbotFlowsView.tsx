...
const ChatbotExecutionsDashboard = lazy(() =>
  import('./ChatbotExecutionsDashboard').then((m) => ({ default: m.ChatbotExecutionsDashboard }))
);
...
export function ChatbotFlowsView() {
  ...
  return (
    <div className="flex h-full flex-col space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Crie e gerencie fluxos de automação de chatbot
          </p>
        </div>
      ...
      <Tabs defaultValue="flows" className="flex min-h-0 flex-1 flex-col">
        ...
        <TabsContent value="flows" className="mt-4 min-h-0 flex-1 space-y-6">
          {/* Stats */}
                  ...
                  {filtered.map((flow) => {
                    ...
                    return (
                      ...
                      >
                        <Card
                          className={cn(
                            'border-secondary/30 transition-all hover:border-primary/30',
                        ...
                        >
                          <CardContent className="p-4">
                            ...
                            <div className="mb-3 flex items-center gap-3 text-xs text-muted-foreground">
                              ...
                              {flow.trigger_value && (
                                <span className="rounded bg-secondary/30 px-2 py-0.5 text-xs">
                                  {flow.trigger_value}
                                </span>
                              )}
                              <span className="flex items-center gap-1">
                                <GitBranch className="h-3 w-3" /> {nodeCount} nós
                              </span>
                              <span className="flex items-center gap-1">
                                <BarChart3 className="h-3 w-3" /> {flow.execution_count || 0} exec.
                              </span>
                            </div>
                            ...
                            <div className="mb-3 flex items-center gap-1 overflow-hidden">
                              {(Array.isArray(flow.nodes) ? flow.nodes : [])
                                .slice(0, 4)
                                .map((node: ChatbotNode, i: number) => (
                                  <div key={node.id || i} className="flex items-center gap-1">
                                    <span className="max-w-20 truncate rounded bg-secondary/30 px-2 py-1 text-xs text-foreground">
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Bot className="h-5 w-5 text-primary" /> Novo Fluxo de Chatbot
            </DialogTitle>
              ...
              <Select
                ...
                onValueChange={(v: string) =>
                  setForm((f) => ({
                    ...f,
                    trigger_type:
                      v as TriggerType /* ignore-audit: Select/Tabs value string narrowed to union; developer controls option values */,
                  }))
                }
              >
                <SelectTrigger>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreate} disabled={!form.name || createFlow.isPending}>
              {createFlow.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
              Criar Fluxo
            </Button>
          </DialogFooter>
        </DialogContent>
