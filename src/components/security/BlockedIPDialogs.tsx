...
export function BlockIPDialog({ open, onClose, onSuccess }: BlockIPDialogProps) {
  ...
  const handleBlockIP = async () => {
    ...
    if (error) {
      toast.error(error.code === '23505' ? 'Este IP já está bloqueado' : 'Erro ao bloquear IP');
    } else {
      ...
      <DialogContent>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleBlockIP} disabled={updating}>
            {updating && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Bloquear
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
    ...
    <AlertDialog open={!!ip} onOpenChange={() => onClose()}>
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            O IP <code className="">{ip?.ip_address}</code> poderá acessar o sistema novamente.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel disabled={updating}>Cancelar</AlertDialogCancel>
          <AlertDialogAction onClick={handleUnblock} disabled={updating}>
            {updating && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Desbloquear
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
