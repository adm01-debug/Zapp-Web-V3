...
export const useDashboardData = (filters: DashboardFilters = getDefaultFilters()) => {
  ...
  const stats: DashboardStats | null = useMemo(() => {
    ...
    // Include contacts without a queue: they are orphaned (never routed) and need
    // attention. Excluding them would hide limbo contacts from supervisors.
    const pendingConversations = contacts.filter((c) => !c.assigned_to).length;
