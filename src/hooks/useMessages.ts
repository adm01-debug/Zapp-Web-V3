...
export function useMessages(remoteJid: string | null, instanceName?: string | null) {
        ...
        (payload) => {
          ...
          setMessages((prev) => {
            if (
              prev.some(
                (m) =>
                  m.id === newMsg.id || (newMsg.message_id && m.message_id === newMsg.message_id)
              )
            ) {
              return prev;
            }
  ...
  const toggleImportant = useCallback(async (id: string, current: boolean) => {
    ...
    try {
      ...
      log.error('toggleImportant error', {
        id,
        error: err instanceof Error ? err.message : String(err),
      });
  ...
  const scheduleFollowUp = useCallback(
    async (id: string, followUpAt: string) => {
      try {
        ...
        log.error('scheduleFollowUp error', {
          id,
          error: err instanceof Error ? err.message : String(err),
        });
      }
  ...
  const markFollowUpDone = useCallback(async (id: string) => {
    try {
      ...
      log.error('markFollowUpDone error', {
        id,
        error: err instanceof Error ? err.message : String(err),
      });
    }
