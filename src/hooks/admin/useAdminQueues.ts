...
export function useAdminQueues() {
  ...
  const load = async () => {
    ...
    const results = await Promise.allSettled([
      ...
      safeClient.from<QueueMember>('queue_members', (query) =>
        query.select('id,queue_id,profile_id,profile:profiles(id,name,avatar_url)')
      ),
      ...
      safeClient.from<ServiceChannel>('service_channels', (query) =>
        query
          .select('id,name,channel_type,default_queue_id')
          .neq('status', 'archived')
          .order('name')
      ),
  ...
  const addMember = async () => {
    ...
    const { error } = await safeClient.from('queue_members', (q) =>
      q.insert({
  ...
  const addSkill = async () => {
    ...
    const { error } = await safeClient.from('queue_skill_requirements', (q) =>
      q.insert({
        queue_id: memberDialog.id,
