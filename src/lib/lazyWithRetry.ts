 ...
 */
const CLOCK_SKEW_TOLERANCE_MS = 60_000; // 60 s
 ...
 *   '-1'          -> -1        -> <0                   -> last=0 -> reload
 *   ''            -> 0         -> isFinite,>=0,≤60s+now -> last=0 -> reload
 *   '1e308'       -> 1e308     -> isFinite,>=0,>60s+now-> last=0 -> reload (FINDING 1)
 *   'now+10min'   -> future ts -> isFinite,>=0,>60s+now-> last=0 -> reload (FINDING 2)
 *   'now-5s'      -> recent    -> isFinite,>=0,≤60s+now -> last=ts-> cooldown
 *   '1750000000'  -> timestamp -> isFinite,>=0,≤60s+now -> last=ts-> cooldown logic
 */
