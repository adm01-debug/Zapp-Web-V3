...
export function useTeamConversations() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public' // ✅ fix: tabelas no schema public (não zapp), table: 'team_messages' }, () => {
      ...
      .on('postgres_changes', { event: '*', schema: 'public' // ✅ fix: tabelas no schema public (não zapp), table: 'team_conversations' }, () => {
        void queryClient.invalidateQueries({ queryKey: ['team-conversations'] });
      })
      .on(
        'postgres_changes',
        { event: '*', schema: 'public' // ✅ fix: tabelas no schema public (não zapp), table: 'team_conversation_members' },
        () => {
          void queryClient.invalidateQueries({ queryKey: ['team-conversations'] });
        }
      )
