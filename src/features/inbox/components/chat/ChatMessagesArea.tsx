...
export const ChatMessagesArea = memo(forwardRef<ChatMessagesAreaRef, ChatMessagesAreaProps>(({
  ...
  return (
    <div 
      ...
      className="flex-1 min-h-0 min-w-0 overflow-y-auto px-4 py-6 md:px-24 scrollbar-none bg-background/20 relative"
    >
      <ChatWatermark />
        ...
        {virtualizer.getVirtualItems().map((virtualRow) => {
          ...
          return (
            <div
              ...
              style={{ position: 'absolute', top: 0, left: 0, width: '100%', transform: `translateY(${virtualRow.start}px)`, paddingBottom: '1rem' }}
            >
              <MessageBubble
                message={message}
                isFirstInGroup={true}
      ...
      {isContactTyping && <div className="mt-4"><TypingIndicator isVisible={true} userName={typingUserName} /></div>}

      <AnimatePresence>
        {showScrollBottom && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className="fixed bottom-24 right-8 z-50">
            <Button size="icon" variant="secondary" className="rounded-full shadow-lg" onClick={() => scrollContainerRef.current?.scrollTo({ top: scrollContainerRef.current.scrollHeight, behavior: 'smooth' })}>
