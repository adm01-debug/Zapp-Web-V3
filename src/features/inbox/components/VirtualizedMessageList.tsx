...
>(
  (
    ...
    const listItems = useMemo((): ListItem[] => {
      ...
      messages.forEach((message) => {
        ...
        if (dateKey !== currentDate) {
          currentDate = dateKey;
          items.push({ type: 'date-separator', date: new Date(dateKey), key: `date-${dateKey}` });
        }
    ...
    const scrollToBottom = useCallback(() => {
      if (listItems.length > 0) virtualizer.scrollToIndex(listItems.length - 1, { align: 'end' });
    ...
    return (
      <div ref={parentRef} className="scrollbar-thin flex-1 overflow-auto bg-muted/5">
        <div
              ...
              >
                {item.type === 'date-separator' ? (
                  <div className="flex justify-center px-4 py-2">
                    <span className="rounded-full border border-border/20 bg-muted/50 px-4 py-1.5 text-xs font-medium text-muted-foreground">
