    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-2xl w-[95vw] p-0 gap-0 bg-background/95 border-border/20 overflow-hidden [&>button]:hidden">
        ...
        <div className="relative flex items-center justify-center min-h-[50vh] max-h-[70vh]">
          {index > 0 && (
            <button type="button" onClick={goPrev} className="absolute left-2 z-10 w-10 h-10 rounded-full bg-muted/60 backdrop-blur flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted/80 transition-all">
              <ChevronLeft className="w-5 h-5" />
            </button>
          )}
          {index < messages.length - 1 && (
            <button type="button" onClick={goNext} className="absolute right-2 z-10 w-10 h-10 rounded-full bg-muted/60 backdrop-blur flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted/80 transition-all">
