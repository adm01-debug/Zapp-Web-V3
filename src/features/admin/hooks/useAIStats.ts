  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AIStats> => {
      ...
      const avgSentimentScore =
        currentAnalyses?.reduce((acc, a) => acc + (a.sentiment_score || 0), 0) /
        (totalAnalyses || 1);
      const positiveSentiment =
        currentAnalyses?.filter((a) => a.sentiment === 'positive').length || 0;
      const negativeSentiment =
        currentAnalyses?.filter((a) => a.sentiment === 'negative').length || 0;
      const neutralSentiment =
        currentAnalyses?.filter((a) => a.sentiment === 'neutral').length || 0;
      ...
      const trendMap = new Map<
        string,
        { scores: number[]; positive: number; negative: number; neutral: number }
      >();
