...
# =============================================================================
grep -rhoiE "alter function (public\.)?[a-zA-Z0-9_]+" "${MIG_DIRS[@]}" 2>/dev/null \
  | sed -E 's/[Aa][Ll][Tt][Ee][Rr] [Ff][Uu][Nn][Cc][Tt][Ii][Oo][Nn] //' | norm | sort -u > "$TMP/fn_altered.txt"

comm -23 "$TMP/fn_altered.txt" "$TMP/fn_defined.txt" > "$TMP/alter_orphans.txt" || true
