...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'connect') {
      ...
      if (response.ok && (data?.qrcode?.base64 ?? data?.base64)) {
        await supabase
          .from('whatsapp_connections')
          .update({ qr_code: (data.qrcode?.base64 ?? data.base64), status: 'qr_pending' })
    ...
    if (action === 'mark-read') {
      ...
      const rm = Array.isArray(jb.readMessages) ? jb.readMessages : [jb.key];
      const mapped = rm.map((it: unknown) => {
        const o = (it && typeof it === 'object') ? it as Record<string, unknown> : {};
        return {
          id: String(o.id ?? o.externalId ?? ''),
          fromMe: o.fromMe === true || o.isSent === true,
          remoteJid: String(o.remoteJid ?? o.contactJid ?? ''),
        };
      }).filter((x) => x.id && x.remoteJid);
      if (mapped.length === 0) return new Response(JSON.stringify({ ok: false, skipped: true, reason: 'missing id/remoteJid' }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      return await proxy(`/chat/markMessageAsRead/${instance}`, 'POST', { readMessages: mapped });
    }
    if (action === 'mark-unread') {
      ...
      if (mapped.length === 0) return new Response(JSON.stringify({ ok: false, skipped: true, reason: 'missing id/remoteJid' }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
      return await proxy(`/chat/markMessageAsUnread/${instance}`, 'POST', { readMessages: mapped });
    }


    if (action === 'read-messages') {
      const jb = body as Record<string, unknown>;
      const remoteJid = String(jb.remoteJid || jb.chat || '');
      if (!remoteJid) {
      ...
      try {
        const found = await proxy(`/chat/findMessages/${instance}`, 'POST', { where: { remoteJid }, limit: 30 });
        if (!found.ok) {
          const ft = await found.text().catch(() => '');
          return new Response(JSON.stringify({ ok: false, skipped: true, upstream_status: found.status, details: ft }), {
            status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }
        const fb = await found.json() as { messages?: { records?: Array<{ key?: { id?: string; fromMe?: boolean; remoteJid?: string } }> } };
        const records = fb?.messages?.records ?? [];
        const toMark = records
          .filter((m) => m?.key && m.key.fromMe === false)
          .map((m) => ({ id: String(m.key?.id ?? ''), fromMe: false, remoteJid: String(m.key?.remoteJid ?? remoteJid) }))
          .filter((x) => x.id);
        if (toMark.length === 0) {
          return new Response(JSON.stringify({ ok: true, skipped: true, reason: 'nothing to mark' }), {
            status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }
        const response = await proxy(`/chat/markMessageAsRead/${instance}`, 'POST', { readMessages: toMark });
    ...
    if (action === 'find-messages') {
      const jb = body as Record<string, unknown>;
      const where = (jb.where && typeof jb.where === 'object' && !Array.isArray(jb.where)) ? { ...(jb.where as Record<string, unknown>) } : {};
      if (jb.remoteJid && !where.remoteJid) where.remoteJid = jb.remoteJid;
      if (jb.timestampStart !== undefined && jb.timestampEnd !== undefined) {
        where.timestamp = { ...((where.timestamp as Record<string, unknown>) || {}), gte: jb.timestampStart, lte: jb.timestampEnd };
      }
      return await proxy(`/chat/findMessages/${instance}`, 'POST', { where, page: jb.page, offset: jb.offset });
    }
