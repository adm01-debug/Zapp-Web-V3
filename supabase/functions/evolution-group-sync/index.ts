...
Deno.serve(async (req) => {
  ...
  if (body.action && body.action !== "groups" && body.action !== "isonwa") {
    return contractViolation422("action", `action inválida: '${body.action}' (esperado 'groups'|'isonwa')`, corsHeaeders);
  }
  ...
  console.log("[evolution-group-sync] groups sync concluído", {
    ...
    errors: stats.errors, primeiro_erro: stats.primeiro_error,
  });
