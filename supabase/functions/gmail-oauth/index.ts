...
serve(async (req) => {
  ...
  const supabase = createClient((Deno.env.get('SELFHOSTED_SUPABASE_URL') ?? Deno.env.get('SUPABASE_URL'))!, (Deno.env.get('SELFHOSTED_SUPABASE_SERVICE_ROLE_KEY', { db: { schema: "zapp" } }) ?? Deno.env.get('SUPABASE_SERVICE_ROLE_KEY'))!);
