...
-- BUG-FIX-CANONICAL-1 (2026-08-04): zapp.messages é uma VIEW — não pode ter índices.
-- Índices movidos para a tabela base evo.evolution_messages.
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_messages_status_created
  ON evo.evolution_messages (status, created_at DESC)
  WHERE status IN ('pending','queued','failed');

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_messages_pending_age
  ON evo.evolution_messages (created_at ASC)
  WHERE status = 'pending';

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_dispatch_errors_created
  ON zapp.dispatch_error_logs (created_at DESC);

-- BUG-FIX-CANONICAL-2 (2026-08-04): tabela renomeada de media_queue para media_download_queue.
CREATE INDEX IF NOT EXISTS idx_media_queue_pending
  ON zapp.media_download_queue (created_at ASC)
  WHERE status = 'pending';
