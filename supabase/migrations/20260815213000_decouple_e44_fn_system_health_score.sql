-- =============================================================================
-- E44 — zapp.fn_system_health_score (Fase 3 — Isolamento I1)
-- =============================================================================
-- Objetivo: remover 6 referências diretas a evo.* (invariante I1).
-- Substituições:
--   evo.evolution_webhook_events_v2 → zapp.evolution_webhook_events_v2
--   evo.evolution_messages          → zapp.evolution_messages
--   evo.evolution_settings          → zapp.evolution_settings
-- search_path: 'evo','pg_catalog','zapp','ops','cron','pg_catalog'
--           → zapp, ops, cron, pg_catalog
-- Nota: public.*, vault.*, ops.*, cron.*, pg_stat_user_tables WHERE schemaname='evo'
--       são referências catalogais — não violam I1.
-- =============================================================================

CREATE OR REPLACE FUNCTION zapp.fn_system_health_score()
 RETURNS jsonb
 LANGUAGE plpgsql
 SET search_path = zapp, ops, cron, pg_catalog
AS $function$
DECLARE
...
BEGIN
  ...
  SELECT MAX(created_at) INTO vt2 FROM zapp.evolution_webhook_events_v2;
  ...
  SELECT COUNT(*) FILTER(WHERE created_at>NOW()-INTERVAL '7 days'),COUNT(*) FILTER(WHERE created_at>NOW()-INTERVAL '24 hours') INTO v_msgs_7d,v_msgs_24h FROM zapp.evolution_messages WHERE instance_name='wpp2';
  ...
  -- 6. r2_storage (10pts)
  v_max:=v_max+10;
  SELECT value->'status' INTO vj FROM zapp.evolution_settings WHERE key='r2_evo_config';
  SELECT value#>>'{}' INTO vs FROM zapp.evolution_settings WHERE key='r2_migration_status';
  ...
  v_max:=v_max+5;
  PERFORM 1 FROM zapp.evolution_webhook_events_v2 LIMIT 1;
  v_score:=v_score+5;
...
COMMENT ON FUNCTION zapp.fn_system_health_score IS
  'Score de saúde do sistema (21 dimensões). '
  'E44 (2026-08-15): evo.evolution_* → zapp.evolution_* (invariante I1, 6 ocorrências). '
  'Acesso restrito: search_path=zapp,ops,cron,pg_catalog.';
