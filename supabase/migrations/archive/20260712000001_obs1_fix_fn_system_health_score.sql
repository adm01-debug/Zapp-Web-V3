-- OBS-1 (2026-07-12): Corrigir fn_system_health_score
--
-- Problemas corrigidos:
-- 1. Seção webhook_pipeline usava evo.evolution_messages_wpp2 (legada/descontinuada)
--    → substituída por evo.evolution_messages WHERE instance_name='wpp2'
--    Bug: GREATEST(vt, vt2) inflava hours_silent pois evolution_messages_wpp2 ficou
--    sem novas mensagens após migração consolidada de 2026-07-11.
--
-- 2. Seção wpp2_connection pontuava 20/20 mesmo com last_connected_at > 2h
--    (DB mantém status='connected' mas reconexão real pode ter caído silenciosamente).
--    → Adicionada penalidade: >2h stale → 12/20 com status='connected_stale'.
--
-- 3. Seção partition_indexes verificava índice 'message_id_instance_name_key' em
--    evolution_messages_wpp2 → substituído por 'uq_msg_msgid_instance' em evolution_messages.
--
-- 4. Seção dead_tuples inspecionava evolution_messages_wpp2 + evolution_webhook_events_wpp2
--    → substituídas por evolution_messages + evolution_webhook_events_v2.
