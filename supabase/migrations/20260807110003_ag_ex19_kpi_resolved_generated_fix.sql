-- ============================================================================
-- AG-EX-19 — fix: coluna generated em evo.evolution_alerts (auto-resolução)
-- 20260807110003_ag_ex19_kpi_resolved_generated_fix.sql
--
-- Problema: evo.evolution_alerts.resolved é GENERATED ALWAYS AS
-- (resolved_at IS NOT NULL) STORED — o UPDATE da auto-resolução do KPI
-- tentava SET resolved = true e falhava em runtime:
--   "column \"resolved\" can only be updated to DEFAULT"
-- (o erro não aparece na criação da função — CREATE FUNCTION não valida o
-- corpo; só explode na 1ª execução do caminho de auto-resolução).
--
-- Fix: remover o SET resolved do UPDATE — a coluna generated se auto-atualiza
-- quando resolved_at é preenchido. Sem mudança de comportamento.
-- ============================================================================
